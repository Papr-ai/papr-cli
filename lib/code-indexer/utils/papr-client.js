  ...
  getClient() {
    if (!this.client) {
      ...
      this.client = new Papr({
        ...
        xAPIKey: apiKey,
        defaultHeaders: {
          // Papr surface attribution. The CLI is bring-your-own-key, so
          // APIKey.created_by_client attributes whichever product provisioned
          // the key, not the CLI. Without this stamp the memory server would
          // attribute CLI writes to `ts_sdk`, since @papr/memory already sends
          // X-Stainless-Lang: js.
          'X-Papr-Client': 'cli',
          // Retained as-is: this is the pre-existing Auth0/QueryLog client_type
          // value, a different taxonomy from the surface above.
    ...
    try {
      ...
      const response = await fetch(`${baseURL}/v1/schemas`, {
        ...
        headers: {
          ...
          'X-API-Key': apiKey,
          // Raw fetch bypasses the SDK, so stamp the surface explicitly.
          'X-Papr-Client': 'cli'
        },
